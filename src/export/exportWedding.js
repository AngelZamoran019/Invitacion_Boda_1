...
export function buildWeddingHTML(project) {
  ...
  const musicElement = music.url ? `<audio id="wedding-music" src="${esc(music.url)}" preload="auto" autoplay loop aria-hidden="true"></audio>` : ''
  ...
  const style = `
    ...
    *{box-sizing:border-box}
    ...
    audio#wedding-music{display:none!important;width:0;height:0;position:absolute;opacity:0;pointer-events:none}
...
${musicElement}<section class="cover" style="${coverStyle}"><div class="ornament" style="${accentPaint}">✦</div><p class="eyebrow" style="${textStyle(labelText)}">${esc(c.eyebrow || '')}</p><h1 style="${textStyle(titleText)}">${esc(title)}</h1>${c.subtitle ? `<p class="cover-subtitle" style="${textStyle(subtitleText)}">${esc(c.subtitle)}</p>` : ''}<div class="line"></div>${c.date ? `<p class="date" style="${textStyle(smallText)}">${esc(c.date)}</p>` : ''}${c.venue ? `<p class="venue" style="${textStyle(smallText)}">${esc(c.venue)}</p>` : ''}<button class="button" style="${textStyle(buttonText)}" onclick="openWeddingInvitation()">${esc(c.buttonLabel || 'Abrir invitación')}</button></section>
...
<section class="closing">${closing.image ? `<img src="${esc(closing.image)}" alt="Cierre">` : ''}<div class="ornament" style="${accentPaint}">✦</div><p class="text" style="${textStyle(paragraphText)}">${esc(closing.message || 'Gracias por ser parte de este momento.')}</p></section></main><script>const weddingMusic=document.getElementById('wedding-music');const startWeddingMusic=()=>{if(!weddingMusic)return;weddingMusic.loop=true;const playResult=weddingMusic.play();if(playResult?.catch)playResult.catch(()=>{})};window.openWeddingInvitation=()=>{startWeddingMusic();document.querySelector('#couple')?.scrollIntoView({behavior:'smooth'})};startWeddingMusic();const target=${JSON.stringify(countdown.targetDate || '')};if(target){const tick=()=>{const d=Math.max(0,new Date(target).getTime()-Date.now());const v=[Math.floor(d/86400000),Math.floor(d/3600000)%24,Math.floor(d/60000)%60,Math.floor(d/1000)%60];['days','hours','minutes','seconds'].forEach((id,i)=>{const e=document.getElementById(id);if(e)e.textContent=String(v[i]).padStart(2,'0')});};tick();setInterval(tick,1000)}document.getElementById('rsvp')?.addEventListener('submit',e=>{e.preventDefault();document.getElementById('rsvp-success').innerHTML='<div class="success">¡Gracias! Tu confirmación quedó registrada en este dispositivo.</div>';e.currentTarget.reset()})</script></body></html>`
}

export default buildWeddingHTML
