...
<section class="section" id="couple">${couplePhoto}<p class="eyebrow" style="${textStyle(labelText)}">Con amor</p><h2 style="${textStyle(sectionText)}">${esc(couple.name1 || 'Nombre')} <span style="${accentPaint}">&</span> ${esc(couple.name2 || 'Nombre')}</h2>${couple.quote ? `<p class="text" style="${textStyle(paragraphText)}">“${esc(couple.quote)}”</p>` : ''}</section>
