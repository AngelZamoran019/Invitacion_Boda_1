...
const FONTS = [
  ['Playfair Display', "'Playfair Display',serif"], ['Cormorant Garamond', "'Cormorant Garamond',serif"], ['Lora', "'Lora',serif"],
  ['Libre Baskerville', "'Libre Baskerville',serif"], ['Montserrat', "'Montserrat',sans-serif"], ['Poppins', "'Poppins',sans-serif"],
  ['Cinzel', "'Cinzel',serif"], ['Great Vibes', "'Great Vibes',cursive"], ['Georgia', 'Georgia,serif'], ['Arial', 'Arial,sans-serif'],
]
...
function StyleSection({ id, title, open, onToggle, mode, color, gradient, set }) {
  ...
  return <section className="appearance-dropdown">
    <button type="button" className="appearance-dropdown-summary" onClick={() => onToggle(id)} aria-expanded={open}>
      <strong>{title}</strong><span className="appearance-current" style={isGradient ? { background: gradient || GRADIENTS[0][1] } : { backgroundColor: safeColor }} /><span>{isGradient ? 'Degradado' : safeColor}</span><b>⌄</b>
    </button>
    {open && <div className="appearance-dropdown-content">
      ...
      {!isGradient ? <div className="appearance-color-editor">
        <div className="appearance-color-picker-wrap"><input className="appearance-color-picker" type="color" value={safeColor} onChange={e => set(`${prefix}Color`, e.target.value)} /><span>Elegir cualquier color</span></div>
        ...
        <div className="appearance-gradient-list">{GRADIENTS.map(([name, value]) => <button key={name} type="button" className="appearance-gradient" style={{ background: value }} onClick={() => { set(`${prefix}Gradient`, value); set(`${prefix}Mode`, 'gradient') }}><span>{name}</span></button>)}</div>
      </div>}
    </div>}
  </section>
}
...
export default function AppearanceControls({ project, set }) {
  ...
  return <div className="appearance-editor">
    ...
    <div className="appearance-card"><strong>Apariencia</strong><small>Configura cada elemento por separado.</small>
      ...
      <section className="appearance-dropdown"><button type="button" className="appearance-dropdown-summary" onClick={() => toggle('font')} aria-expanded={open === 'font'}><strong>Tipografía</strong><span>{FONTS.find(f => f[1] === a.fontFamily)?.[0] || 'Playfair Display'}</span><b>⌄</b></button>{open === 'font' && <div className="appearance-dropdown-content"><select value={a.fontFamily || FONTS[0][1]} onChange={e => set('appearance.fontFamily', e.target.value)}>{FONTS.map(([name, value]) => <option key={value} value={value}>{name}</option>)}</select><div className="appearance-font-preview" style={{ fontFamily: a.fontFamily || FONTS[0][1] }}>Nuestra historia • Ana & Carlos</div></div>}</section>
    </div>
  </div>
}
