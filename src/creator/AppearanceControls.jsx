...
const TYPE_ITEMS = [
  ...
  ['label', 'Etiquetas / encabezados pequeños'],
  ['small', 'Texto pequeño'],
  ['button', 'Botones'],
]
...
const DEFAULTS = {
  ...
  button: { fontFamily: FONTS[9][1], fontSize: 13, color: '#ffffff', mode: 'solid', gradient: '', fontWeight: 600, lineHeight: 1.2, letterSpacing: 0.5 },
}
  ...
  return (
    <div className="appearance-color-editor">
      <div className="appearance-color-picker-wrap">
        <input className="appearance-color-picker" type="color" value={safe} onChange={e => onChange(e.target.value)} />
        <span>Elegir cualquier color</span>
      </div>
      <input className="appearance-hex-input" value={value || ''} onChange={e => onChange(e.target.value)} placeholder="#FFFFFF" />
    </div>
  )
}
...
function GradientBuilder({ value, onChange }) {
  ...
  const sync = (nextColors, nextType = type, nextAngle = angle) => {
    ...
    onChange(nextType === 'linear'
      ? `linear-gradient(${nextAngle}deg, ${nextColors.join(', ')})`
      : `radial-gradient(${nextColors.join(', ')})`)
  }

  return (
    <div className="appearance-gradient-builder">
      <label>Tipo
        <select value={type} onChange={e => { const next = e.target.value; setType(next); sync(colors, next) }}>
      ...
      {type === 'linear' && (
        <label>Dirección
          <select value={angle} onChange={e => { const next = e.target.value; setAngle(next); sync(colors, type, next) }}>
      ...
      {colors.map((color, index) => (
        <div className="appearance-gradient-stop" key={`${index}-${color}`}>
          <input type="color" value={color} onChange={e => { const next = [...colors]; next[index] = e.target.value; sync(next) }} />
          <input value={color} onChange={e => { const next = [...colors]; next[index] = e.target.value; sync(next) }} />
      ...
      <div className="appearance-gradient-list">
        {GRADIENTS.map(([name, gradient]) => (
          <button
            key={name}
            type="button"
            className="appearance-gradient"
            style={{ background: gradient }}
            onClick={() => {
              const next = gradientColors(gradient)
              setColors(next)
              setType('linear')
              setAngle('135')
              onChange(gradient)
...
function BaseStyle({ id, title, appearance, set, open, onToggle }) {
  ...
  const color = appearance[`${id}Color`] || '#ffffff'
  const gradient = appearance[`${id}Gradient`] || ''
  ...
  return (
    <section className="appearance-dropdown">
      <button type="button" className="appearance-dropdown-summary" onClick={() => onToggle(id)} aria-expanded={open}>
        <strong>{title}</strong>
        <span className="appearance-current" style={{ background: preview }} />
        <span>{isGradient ? 'Degradado' : color}</span>
        <b>⌄</b>
      </button>
      ...
      {open && (
        <div className="appearance-dropdown-content">
          <div className="appearance-style-switch">
            <button type="button" className={mode === 'solid' ? 'active' : ''} onClick={() => set(`appearance.${id}Mode`, 'solid')}>Color</button>
            <button type="button" className={mode === 'gradient' ? 'active' : ''} onClick={() => set(`appearance.${id}Mode`, 'gradient')}>Degradado</button>
          </div>

          {mode === 'gradient' ? (
            <GradientBuilder value={gradient} onChange={value => { set(`appearance.${id}Gradient`, value); set(`appearance.${id}Mode`, 'gradient') }} />
          ) : (
            <ColorPicker value={color} onChange={value => { set(`appearance.${id}Color`, value); set(`appearance.${id}Mode`, 'solid') }} />
          )}
        </div>
      )}
    </section>
...
function Typography({ project, set, open, onToggle }) {
  ...
  const gradient = current.mode === 'gradient' && Boolean(current.gradient)
        ...
        <div className="appearance-dropdown-content">
          <label>Elemento
            <select value={item} onChange={e => setItem(e.target.value)}>
              {TYPE_ITEMS.map(([id, label]) => <option key={id} value={id}>{label}</option>)}
            </select>
          </label>

          <label>Fuente
            <select value={current.fontFamily} onChange={e => write('fontFamily', e.target.value)}>
              {FONTS.map(([name, value]) => <option key={value} value={value}>{name}</option>)}
            </select>
          </label>

          <div className="appearance-type-grid">
            <label>Tamaño<input type="number" min="8" max="120" value={current.fontSize} onChange={e => write('fontSize', Number(e.target.value))} /></label>
            <label>Peso<select value={current.fontWeight} onChange={e => write('fontWeight', Number(e.target.value))}><option value="300">Ligero</option><option value="400">Normal</option><option value="500">Medio</option><option value="600">Seminegrita</option><option value="700">Negrita</option></select></label>
            <label>Interlineado<input type="number" min="0.8" max="3" step="0.05" value={current.lineHeight} onChange={e => write('lineHeight', Number(e.target.value))} /></label>
            <label>Espaciado<input type="number" min="-5" max="20" step="0.5" value={current.letterSpacing} onChange={e => write('letterSpacing', Number(e.target.value))} /></label>
          </div>

          <div className="appearance-style-switch">
            <button type="button" className={!gradient ? 'active' : ''} onClick={() => write('mode', 'solid')}>Color</button>
            <button type="button" className={gradient ? 'active' : ''} onClick={() => write('mode', 'gradient')}>Degradado</button>
          </div>

          {gradient ? (
            <GradientBuilder value={current.gradient} onChange={value => { write('gradient', value); write('mode', 'gradient') }} />
          ) : (
            <ColorPicker value={current.color} onChange={value => { write('color', value); write('mode', 'solid') }} />
          )}

          <div
            className="appearance-live-type"
            style={{
              ...
              background: paint(current),
              WebkitBackgroundClip: gradient ? 'text' : 'initial',
              backgroundClip: gradient ? 'text' : 'initial',
              color: gradient ? 'transparent' : current.color,
            }}
          >
            Vista previa de {TYPE_ITEMS.find(([id]) => id === item)?.[1]}
          </div>
        </div>
      )}
    </section>
  ...
  return (
    <div className="appearance-editor">
      ...
      <div className="appearance-card">
        ...
        <small>Control total del diseño, por elemento.</small>
        <BaseStyle id="background" title="Fondo" appearance={appearance} set={set} open={open === 'background'} onToggle={toggle} />
        <BaseStyle id="accent" title="Acento" appearance={appearance} set={set} open={open === 'accent'} onToggle={toggle} />
        <BaseStyle id="text" title="Texto base" appearance={appearance} set={set} open={open === 'text'} onToggle={toggle} />
