...
const paint = (style) => (
  style.mode === 'gradient' && style.gradient ? style.gradient : style.color || '#fff'
)
...
function GradientBuilder({ value, onChange }) {
  ...
  const [colors, setColors] = useState(
    matches?.length ? matches : ['#8b6f47', '#c9a86a', '#f4efe8'],
  )
  const [type, setType] = useState(
    value?.startsWith('radial-gradient') ? 'radial' : 'linear',
  )
  ...
  return (
    <div className="appearance-gradient-builder">
      ...
      <div
        ...
        style={{ background: value || GRADIENTS[0][1] }}
      />
      ...
      {colors.map((color, index) => (
        <div className="appearance-gradient-stop" key={`${index}-${color}`}>
          ...
          <button
            ...
            onClick={() => sync(colors.filter((_, colorIndex) => colorIndex !== index))}
          >
            ×
          </button>
        </div>
      ...
      <button
        type="button"
        className="appearance-add-color"
        onClick={() => sync([...colors, '#ffffff'])}
      >
        + Agregar color
      </button>
      ...
      <div className="appearance-gradient-list">
        {GRADIENTS.map(([name, gradient]) => (
          <button
            ...
            onClick={() => {
              const nextColors = gradient.match(/#[0-9a-fA-F]{6}/g) || colors
              setColors(nextColors)
              setType('linear')
              setAngle('135')
              onChange(gradient)
...
function BaseStyle({ id, title, a, set, open, onToggle }) {
  const mode = a[`${id}Mode`] || 'solid'
  const color = a[`${id}Color`] || '#ffffff'
  const gradient = a[`${id}Gradient`] || ''

  return (
    <section className="appearance-dropdown">
      <button
        type="button"
        className="appearance-dropdown-summary"
        onClick={() => onToggle(id)}
      >
        <strong>{title}</strong>
        <span
          className="appearance-current"
          style={{ background: mode === 'gradient' && gradient ? gradient : color }}
        />
        ...
        <div className="appearance-dropdown-content">
          <div className="appearance-style-switch">
            <button
              ...
              onClick={() => set(`appearance.${id}Mode`, 'solid')}
            >
              Color
            </button>
            <button
              ...
              onClick={() => set(`appearance.${id}Mode`, 'gradient')}
            >
              Degradado
            </button>
          </div>
          ...
          {mode === 'gradient' ? (
            <GradientBuilder
              value={gradient}
              onChange={(nextGradient) => {
                set(`appearance.${id}Gradient`, nextGradient)
            ...
            <ColorPicker
              value={color}
              onChange={(nextColor) => {
                set(`appearance.${id}Color`, nextColor)
                set(`appearance.${id}Mode`, 'solid')
              }}
            />
          )}
          ...
          {id === 'background' && (
            <>
              <h4>Textura del fondo</h4>
              <div className="appearance-texture-list">
                {TEXTURES.map(([name, texture]) => (
                  <button
                    key={name}
                    type="button"
                    className={a.backgroundTexture === texture ? 'selected' : ''}
                    style={texture !== 'none' ? { backgroundImage: texture } : undefined}
                    onClick={() => set('appearance.backgroundTexture', texture)}
                  >
                    {name}
                  </button>
                ))}
              ...
              <label>
                ...
                <input
                  ...
                  max="0.5"
                  step="0.01"
                  value={a.backgroundTextureOpacity ?? 0.12}
                  onChange={(event) => set(
                    'appearance.backgroundTextureOpacity',
...
function Typography({ project, set, open, onToggle }) {
  ...
  const gradient = current.mode === 'gradient'
        ...
        <div className="appearance-dropdown-content">
          ...
          <label>
            ...
            <select
              ...
              onChange={(event) => write('fontFamily', event.target.value)}
            >
          ...
          <div className="appearance-type-grid">
            <label>
              ...
              <input
                ...
                onChange={(event) => write('fontSize', Number(event.target.value))}
              />
            </label>
            <label>
              Peso
            ...
            <label>
              ...
              <input
                ...
                onChange={(event) => write('lineHeight', Number(event.target.value))}
              />
            </label>
            <label>
              Espaciado
              <input
                ...
                onChange={(event) => write('letterSpacing', Number(event.target.value))}
              />
            </label>
          </div>
