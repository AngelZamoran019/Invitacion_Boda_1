    ...
    <div className="editor-layout">
      <aside className="editor-sidebar">
        {sectionGroups.map((group) => (
          <div key={group.id} className="editor-group">
            <button type="button" className="editor-group-title" onClick={() => toggleGroup(group.id)}><span>{group.title}</span><b aria-hidden="true">{openGroups[group.id] ? '⌃' : '⌄'}</b></button>
            {openGroups[group.id] && <div className="editor-group-items">{group.items.map(([id, label]) => <button key={id} type="button" className={activeSection === id ? 'editor-nav-item active' : 'editor-nav-item'} onClick={() => selectSection(group.id, id)}>{label}</button>)}</div>}
          </div>
        ))}
      </aside>
