...
const applyConfirmationTexts = (html, project) => {
  ...
  const target = sections.find(section =>
    ...
    /<form\s+[^>]*id=["']rsvp["']/i.test(section) ||
    /<div\s+id=["']rsvp-success["']/i.test(section)
  ) || ''
...
const applyConfirmationContentStyles = (html, project) => {
  ...
  const source = String(html || '')
  const target = /<section\s+class=["']section["'][^>]*(?:id=["']confirmacion["'][^>]*)>[\s\S]*?<\/section>/i.exec(source)?.[0]
    || /<section\s+class=["']section["'][^>]*>[\s\S]*?(?:<form\s+[^>]*class=["'][^"']*\brsvp-form\b[^"']*["'][\s\S]*?<\/form>|<div\s+id=["']rsvp-success["'][^>]*>[\s\S]*?<\/div>)[\s\S]*?<\/section>/i.exec(source)?.[0]
  if (!target) return source
...
function PreviewWedding({ project, refreshKey = 0 }) {
  ...
  useEffect(() => {
    ...
    const syncProjectToIframe = (nextProject) => {
      ...
      try {
        ...
        const html = applyConfirmationContentStyles(applyGiftsContentStyles(applyGiftsContent(baseHTML, nextProject), nextProject), nextProject)
  ...
  window.addEventListener('message',event=>{
    ...
    try {
      ...
      const html = buildBridgeHTML(applyConfirmationContentStyles(applyGiftsContentStyles(applyGiftsContent(baseHTML, projectRef.current), projectRef.current), projectRef.current))
  ...
  useEffect(() => {
    ...
    const html = applyConfirmationContentStyles(applyGiftsContentStyles(applyGiftsContent(baseHTML, project), project), project)
    iframe.contentWindow?.postMessage({ type: 'WEDDING_PREVIEW_UPDATE', html }, '*')
  }, [project])
